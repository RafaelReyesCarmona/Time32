...
time32_t  prevEventTime[nbrInputPins] ;    // the time of the previous event
...
void loop()
{
   for (int i=0; i < nbrInputPins; i++) {
     ...
     if (val != state[i]) {
        time32_t duration = 0; // the time since the previous event
        state[i] = val;
        time32_t timeNow = now();
...
void logEvent( int pin, boolean state, time32_t timeNow, time32_t duration)
{
   Serial.print("Pin ");
   Serial.print(pin);
   if (state == HIGH) {
...
void showDuration(time32_t duration)
{
