/*
  TimeLib32.h - low level time and date functions for ESP32 and Arduino.
*/

/*
  July 3 2011 - fixed elapsedSecsThisWeek macro (thanks Vincent Valdy for this)
              - fixed  daysToTime_t macro (thanks maniacbug)
  July 14 2022 - fixed "conflicting declaration 'typedef time_t'" on ESP32 enviroment. 
...
typedef time32_t(*getExternalTime)();
...
/* Useful Macros for converting elapsed time to a time32_t */
...
uint8_t isAM(time32_t t);    // returns true the given time is AM
uint8_t isPM();            // returns true if time now is PM
uint8_t isPM(time32_t t);    // returns true the given time is PM
int     minute();          // the minute now 
int     minute(time32_t t);  // the minute for the given time
int     second();          // the second now 
int     second(time32_t t);  // the second for the given time
int     day();             // the day now 
int     day(time32_t t);     // the day for the given time
